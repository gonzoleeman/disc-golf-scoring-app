...
def score_round(rd_list):
    ...
    for rnd in rd_list:
        ...
        rnd.calc_score = float(front_score_results[idx])
        idx = 0
        for brnd in back_list:
            ...
            if rnd == brnd:
                break
            idx += 1
        rnd.calc_score += float(back_score_results[idx])
        idx = 0
        for trnd in ttl_list:
            ...
            if rnd == trnd:
                break
            idx += 1
        rnd.calc_score += float(ttl_score_results[idx])
        dprint("Score for this round/player: %f" % rnd.calc_score)
        #result_rd_list.append(rnd)

    #dprint("Returning overall results:", result_rd_list)
    #for rd in result_rd_list:
    #    dprint("Overall[]:", rd)
    #return result_rd_list
